...
import { CheckIcon, Loader2Icon, RefreshCcwIcon, UploadIcon, XIcon } from "lucide-react";
...
export function ImportReviewDialog<TRow extends ImportReviewRow>({
    ...
    async function handleFile(file: File) {
        ...
        try {
            ...
            toast.error(
                error instanceof Error ? error.message : t("dataTable.importPreviewFailed"),
            );
        } finally {
            setIsPreviewing(false);
        }
    ...
    const filteredRows = useMemo(() => {
        return reviewRows.filter((row) => filter === "all" || row.status === filter);
    }, [filter, reviewRows]);
                    ...
                    <div className="flex flex-col gap-4 px-4 py-4">
                        ...
                        {ignoredColumns.length > 0 ? (
                            <p className="text-xs text-muted-foreground">
                                {t("dataTable.importIgnoredColumns")} {ignoredColumns.join(", ")}
                            </p>
                        ) : null}
...
export function ImportReviewStatusBadge({ status }: { status: ImportReviewRow["status"] }) {
    const { t } = useTranslation();
